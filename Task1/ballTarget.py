"""
Ball Trajectory Simulation Using Numerical Methods
Author: annanoaa
Date: 2025-01-19
Description: Simulates ball trajectories to hit fixed targets using numerical methods.
"""
...
class TrajectoryCalculator:
    ...
    def find_trajectory(self,
        ...
        angle_ranges = np.linspace(0.1, np.pi / 2 - 0.1, 40)
...
class TrajectoryAnimator:
    ...
    def __init__(self, fig_size: Tuple[int, int] = (10, 6)):
            ...
            if traj_idx < len(trajectories):
                ...
                if points_to_show > 0:
                    self.ax.plot(x_points[:points_to_show],
                                 y_points[:points_to_show], 'b-', linewidth=2)
                    self.ax.plot(x_points[points_to_show - 1],
                                 y_points[points_to_show - 1], 'bo', markersize=8)
...
def main():
    ...
    try:
        ...
        # Calculate trajectories - always start from initial position (0,0)
        trajectories = []
        starting_point = Point(0, 0)  # Fixed starting point
